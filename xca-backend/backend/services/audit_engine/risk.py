...
Risk Classification Logic (per PRD):
    ...
    - Minor issue only           -> MEDIUM RISK -> ITC APPROVED (warning)
    - Major break (GSTR-3B / tax unpaid / IRN invalid) -> HIGH RISK -> ITC REJECTED
    - All checkpoints fail       -> CRITICAL    -> ITC REJECTED
"""

from .models import Checkpoint, CheckpointStatus, RiskLevel, Decision
